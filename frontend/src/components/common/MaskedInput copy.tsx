...
const MaskedInput: React.FC<MaskedInputProps> = ({
  ...
  const getCalculatedKeyboardType = (type?: MaskType): KeyboardTypeOptions => {
    ...
    if (type === 'brl' || type === 'cpf' || type === 'phone' || type === 'cnpj' || type === 'cep' || type === 'creditCard' || type === 'cardExpiry' || type === 'cvv' || type === 'date' || type === 'time') {
      return 'numeric';
    }
    return 'default';
  };
  const finalKeyboardType = externalKeyboardType || getCalculatedKeyboardType(maskType);
  ...
  const getKeyboardType = (type: MaskType): KeyboardTypeOptions => {
    if (type === 'brl' || type === 'cpf' || type === 'phone' || type === 'cnpj' || type === 'cep' || type === 'creditCard' || type === 'cardExpiry' || type === 'cvv' || type === 'date') {
      return 'numeric';
    }
    return 'default';
  };

  const keyboardType = getKeyboardType(maskType);

  return (
    <View style={[maskedInputStyles.container, variant === 'compact' && maskedInputStyles.compactContainer,]}>
    ...
    <TextInput
      style={[maskedInputStyles.input, variant === 'compact' && maskedInputStyles.compactInputText]}
